# remote state terraform buckets 
terraform {
  backend "s3" {
    bucket = "tf-state-nonprod"
    key    = "github.com/duttpathak/terraform_learn/ec2_instance"
    region = "us-west-2"
  }
}
# module call 
# has to be parent directory with the word modules in it. 
# quotes makes string



module "ec2-instance" {
  source              = "/Users/dutt/Code/terraform_learn/terraform_modules/single_ec2_instance"
  instance_type       = "t3.micro"
  ami                 = var.ami
  key_name            = "aws_key_pair"
  public_key          = "ssh-rsa  tarpanpathak720@gmail.com"
  security_group_name = "terraform-example-instance"
  user_data           = <<-EOF
        #!/bin/bash
        set -ex

        dnf update -y
        # install the http server 
        dnf install -y httpd
        # starts the http server
        systemctl start httpd
        EOF
}

module "asg" {
  source = "/Users/dutt/Code/terraform_learn/terraform_modules/autoscaling_group"
  # instance_type       = "t3.micro"
  image_id            = "ami-0889a44b331db0194"
  key_name            = "aws_key_pair"
  public_key          = "ssh-rsa  tarpanpathak720@gmail.com"
  security_group_name = "terraform-asg-example"
  health_check_type   = "ELB"
  vpc_id              = "vpc-2a7d1051"
  health_check_path   = "/"

  user_data = <<-EOF
#!/bin/bash
set -ex

dnf update -y
# install the http server 
dnf install -y httpd
# starts the http server
systemctl start httpd
EOF
  tags = {
    Name = "terraform_asg"
  }
}



